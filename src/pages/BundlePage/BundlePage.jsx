...
export default function BundlePage() {
  ...
  // NEW: State for multiple company matches
  const [multipleMatches, setMultipleMatches] = useState([]);
  // NEW: Track which company was selected from the number (for bundle filtering)
  const [selectedCompanyFromNumber, setSelectedCompanyFromNumber] = useState("");
  ...
  // Load bundles when filters change
  useEffect(() => {
    if (!isLoading) {
      // Use selectedCompanyFromNumber for bundle filtering when available, otherwise use companyId
      const bundleCompanyId = selectedCompanyFromNumber || companyId;
      dispatch(getBundles(page, rowsPerPage, countryId, validity, bundleCompanyId, categoryId, searchTag));
    }
  }, [validity, companyId, selectedCompanyFromNumber, searchTag, page, rowsPerPage, countryId, categoryId, isLoading]);
  ...
  // MODIFIED: filteredServiceList - now shows matching companies when multiple matches exist
  const filteredServiceList = useMemo(() => {
    if (type == "social") return serviceList;
    if ((number.length < 3 && number.length >= 0)) return serviceList;

    // If we have a companyId selected, filter by it
    if (companyId) {
      return serviceList.filter(service => service.company.id === companyId);
    }

    // If we have a number with multiple matches, show only those matching companies
    if (number.length >= 3 && multipleMatches.length > 0) {
      return multipleMatches;
    }

    return serviceList;
  }, [companyId, serviceList, number.length, multipleMatches]);

  // NEW: Function to find ALL matching services
  const findAllMatchingServices = (phoneNumber) => {
    ...
    for (const service of serviceList) {
      if (service?.company?.companycodes) {
        for (const code of service.company.companycodes) {
          const reservedLength = code.reserved_digit?.length || 0;
          if (reservedLength > 0 && phoneNumber.length >= reservedLength) {
            const prefix = phoneNumber.substring(0, reservedLength);
            if (prefix === code.reserved_digit) {
    ...
    for (const service of matches) {
      if (!companyIds.has(service.company.id)) {
        companyIds.add(service.company.id);
        uniqueMatches.push(service);
      }
    }
  ...
  // MODIFIED: Handle company detection with multiple matches
  useEffect(() => {
    if (number.length >= 3) {
      const matches = findAllMatchingServices(number);

      if (matches.length > 1) {
        setMultipleMatches(matches);
        setCompanyId("");
        setSelectedCompanyFromNumber(""); // Clear selected company when multiple matches
        if (number.length === parseInt(phoneNumberLength)) {
          setPhoneNumberError("multiple_companies");
        } else {
          setPhoneNumberError("");
        }
      } else if (matches.length === 1) {
        setMultipleMatches([]);
        const matchedCompanyId = matches[0]?.company?.id;
        setCompanyId(matchedCompanyId);
        setSelectedCompanyFromNumber(matchedCompanyId); // Store for bundle filtering
        setPhoneNumberError("");
      } else {
        setMultipleMatches([]);
        setSelectedCompanyFromNumber(""); // Clear on no match
        if (number.length === parseInt(phoneNumberLength)) {
          setCompanyId("");
          setPhoneNumberError("Invalid Phone");
        }
      ...
      setPhoneNumberError("")
      setMultipleMatches([]);
      setSelectedCompanyFromNumber("");
    }
  }, [number, serviceList, phoneNumberLength]);
  ...
  // MODIFIED: Handle bundle selection with multiple matches
  const handleBundleSelect = (bundle) => {
    if (number.length >= 3 && number.length === parseInt(phoneNumberLength)) {
      const matches = findAllMatchingServices(number);
      ...
      if (matches.length > 1) {
        setMultipleMatches(matches);
        setPhoneNumberError("multiple_companies");
        toast.error("Multiple companies match this number. Please select one from the buttons above.");
        return;
      }

      if (matches[0]?.company?.id !== bundle?.service?.company_id) {
        toast.error("Phone number doesn't match the selected bundle's company");
        return;
      }
    }
  ...
  // MODIFIED: Handle number change with multiple matches
  const handleNumberChange = (e) => {
    const value = e.target.value;
    setNumber(value);
    ...
    if (value.length === 0) {
      ...
      const matches = findAllMatchingServices(value);
      ...
      if (matches.length === 0) {
        ...
        const matchedCompanyId = matches[0]?.company?.id;
        setCompanyId(matchedCompanyId);
        setSelectedCompanyFromNumber(matchedCompanyId);
        setMultipleMatches([]);
      } else if (matches.length > 1) {
        setMultipleMatches(matches);
        setPhoneNumberError("multiple_companies");
          ...
          for (const code of service.company.companycodes) {
            ...
            if (reservedLength > 0 && value.length >= reservedLength) {
              ...
              if (prefix === code.reserved_digit) {
                ...
                if (reservedLength > matchedPrefix.length) {
                  matchedPrefix = code.reserved_digit;
                }
              }
            }
      ...
      for (const service of matchedServices) {
        if (!seenIds.has(service.company.id)) {
          seenIds.add(service.company.id);
          uniqueServices.push(service);
        }
      }

      if (selectedBundle) {
        const isMatch = uniqueServices.some(s => s.company.id === selectedBundle?.service?.company_id);
        if (!isMatch && uniqueServices.length > 0) {
          setPhoneNumberError("Invalid Phone");
        } else if (uniqueServices.length > 1) {
          setMultipleMatches(uniqueServices);
          setPhoneNumberError("multiple_companies");
          setSelectedCompanyFromNumber("");
        } else {
          setPhoneNumberError("");
        ...
        setPhoneNumberError("");
        const matchedCompanyId = uniqueServices[0]?.company?.id;
        setCompanyId(matchedCompanyId);
        setSelectedCompanyFromNumber(matchedCompanyId);
        setMultipleMatches([]);
      } else if (uniqueServices.length > 1) {
  ...
  // NEW: Handle company selection from multiple matches
  const handleCompanySelect = (selectedService) => {
    const selectedId = selectedService.company.id;
    setCompanyId(selectedId);
    setSelectedCompanyFromNumber(selectedId); // IMPORTANT: This will trigger bundle filtering
    setMultipleMatches([]);
  ...
  // NEW: State for company selector visibility (for the popup)
      ...
      <div className="grid grid-cols-12 gap-4 md:gap-6">
        ...
        <div className="col-span-12 space-y-6 xl:col-span-12">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            ...
            <div className="relative">
              ...
              {type == "nonsocial" && (
                <Input
                  ...
                  inputMode="numeric"
                  pattern="[0-9]*"
                  error={phoneNumberError && phoneNumberError !== "multiple_companies"}
                  hint={phoneNumberError === "multiple_companies" ? "Multiple companies match this number. Please select one from above" : phoneNumberError}
                  placeholder={t("ENTER_YOUR_NUMBER")}
                  helperText={phoneNumberError === "multiple_companies" ? "Multiple companies match this number. Please select one from above" : phoneNumberError}
                  required
                  inputProps={{
                    min: 0,
                  }}
                  className={`h-11 rounded-lg border ${phoneNumberError && phoneNumberError !== "multiple_companies" ? 'border-red-500' : 'border-gray-200'} bg-transparent py-2.5 pl-12 pr-14 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:outline-none focus:ring ${phoneNumberError && phoneNumberError !== "multiple_companies" ? 'focus:border-red-500 focus:ring-red-500/10' : 'focus:border-brand-300 focus:ring-brand-500/10'
                    } dark:border-gray-800 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800`}
                />
              )}
            ...
            {/* ORIGINAL SERVICE BUTTONS - COMPLETELY UNCHANGED */}
            <div className="relative">
              <div className="flex gap-2 overflow-x-auto pb-3 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
                {filteredServiceList.map((service, index) => (
                  <button
                    key={index}
                    onClick={() => handleCompanySelect(service)}
                    className={`flex-shrink-0 px-4 py-2 text-[16px] font-medium rounded-lg border transition-all
                    ${companyId === service?.company?.id
                        ? "bg-gradient-to-r from-purple-300 to-blue-300 text-gray-900 shadow-md"
                        : "border-gray-300 text-gray-700 hover:bg-gray-100"
          ...
          <div className="bg-white p-4 rounded-xl shadow-xl w-80">
            ...
            <div className="mt-3">
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full p-2 pl-9 text-sm border rounded-lg focus:outline-none transition-all ${modalErrors.number || (phoneNumberError && phoneNumberError !== "multiple_companies")
                    ? 'border-red-300 bg-red-50 focus:border-red-500'
                    : 'border-gray-200 bg-gray-50 focus:border-purple-500 focus:bg-white'
                    }`}
                  maxLength={phoneNumberLength}
              ...
              {(modalErrors.number || (phoneNumberError && phoneNumberError !== "multiple_companies")) && (
                <p className="text-red-500 text-xs mt-1 flex items-center gap-1">
                  <span className="text-red-500 text-xs">●</span> {modalErrors.number || phoneNumberError}
                </p>
              )}
            ...
            {loading ? (
              ...
              <div className="flex gap-2 mt-4">
                <button
                  onClick={checkPIN}
                  disabled={multipleMatches.length > 0}
                  className={`flex-1 bg-gradient-to-r from-green-500 to-green-600 text-white py-2.5 rounded-lg font-medium text-sm transition-all shadow-sm shadow-green-500/30
                    ${multipleMatches.length > 0 ? 'opacity-50 cursor-not-allowed' : 'hover:from-green-600 hover:to-green-700'}`}
                >
                  <span className="flex items-center justify-center gap-1.5">
                    <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
