...
export default function BundlePage() {
  ...
  // NEW: Helper function to find ALL matching services for a given number (for multiple prefix handling)
  const findAllMatchingServices = (phoneNumber, services) => {
    ...
    for (const service of services) {
      if (service?.company?.companycodes) {
        for (const code of service.company.companycodes) {
          const prefix = code.reserved_digit;
          if (prefix && phoneNumber.startsWith(prefix)) {
            ...
            break; // Only add each company once
          }
        }
      }
    }
    ...
    return matches.sort((a, b) => b.prefixLength - a.prefixLength);
  };

  // NEW: Helper function to find the best matching service (longest prefix, and prompt if multiple)
  const findBestMatchingService = (phoneNumber, services, selectedBundleParam = null) => {
    if (!phoneNumber || phoneNumber.length < 3) return null;

    const allMatches = findAllMatchingServices(phoneNumber, services);

    if (allMatches.length === 0) return null;

    // If a bundle is selected, prefer that company
    if (selectedBundleParam) {
      const bundleMatch = allMatches.find(match => match.company.id === selectedBundleParam.service.company_id);
      if (bundleMatch) return bundleMatch.service;
    }

    // Get matches with the longest prefix length
    const longestLength = allMatches[0].prefixLength;
    const bestMatches = allMatches.filter(match => match.prefixLength === longestLength);

    // If multiple companies have the same longest prefix, show selection dialog
    if (bestMatches.length > 1) {
      return { multiple: true, matches: bestMatches };
    }

    return bestMatches[0].service;
  };
...
const filteredServiceList = useMemo(() => {
  ...
  // If number length is less than 3, show all services
  if (number.length < 3) return serviceList;

  // If we have a companyId selected, show only that company
  if (companyId) {
    return serviceList.filter(service => service.company.id === companyId);
  }

  // If number length is 3 or more but no companyId selected (multiple matches case)
  // Show only companies that match the prefix
  if (number.length >= 3) {
      ...
      if (service?.company?.companycodes) {
        for (const code of service.company.companycodes) {
          ...
          if (prefix && number.startsWith(prefix)) {
            matchingCompanyIds.add(service.company.id);
            break;
          }
        }
      }
    }
    return serviceList.filter(service => matchingCompanyIds.has(service.company.id));
  }

  return serviceList;
}, [companyId, serviceList, number.length]);

  // MODIFIED: Handle company detection with multiple prefix support
  useEffect(() => {
    if (number.length >= 3) {
      const bestMatch = findBestMatchingService(number, serviceList, selectedBundle);

      if (bestMatch === null) {
        setCompanyId("");
      } else if (bestMatch.multiple) {
        // Multiple companies found with same prefix - don't auto-select, show toast
        const companyNames = bestMatch.matches.map(m => m.company.company_name).join(", ");
        toast.warning(`Multiple operators found (${companyNames}). Please select one from the list above.`, {
          autoClose: 5000
        });
        setCompanyId(""); // Don't auto-select, let user choose
      } else {
        setCompanyId(bestMatch?.company?.id);
      }
    } else if (number.length < 3 && number.length > 0) {
      setCompanyId("")
    }
  }, [number, serviceList, selectedBundle]);
  ...
  // MODIFIED: Handle number change with multiple prefix support
  const handleNumberChange = (e) => {
    const value = e.target.value;
    setNumber(value);
    ...
    if (value.length === 0) {
      ...
      if (selectedBundle) {
        ...
        // NEW: Check if there are multiple possible matches
        const allMatches = findAllMatchingServices(value, serviceList);
        if (allMatches.length > 1) {
          // Multiple matches found - don't show error, just prompt user to select
          const companyNames = allMatches.map(m => m.company.company_name).join(", ");
          setPhoneNumberError(`Multiple operators (${companyNames}). Please select one from the list.`);
          setCompanyId(""); // Clear company ID so user can select
        } else {
          setPhoneNumberError("Invalid Phone");
        }
      } else {
