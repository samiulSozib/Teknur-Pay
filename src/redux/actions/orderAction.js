...
export const getOrders = (page, items_per_page, filterStatus, order_type, search_target) => {
    return async (dispatch) => {

        dispatch({ type: ORDER_LIST_REQUEST })
        try {
            const token = localStorage.getItem('token');
            const orders_url = `${base_url}/orders?page=${page}&per_page=${items_per_page}` +
                (filterStatus ? `&order_status=${filterStatus}` : '') +
                (order_type ? `&order_type=${order_type}` : ``) + (search_target ? `&search_target=${search_target}` : "");;
            const config = {
                headers: {
