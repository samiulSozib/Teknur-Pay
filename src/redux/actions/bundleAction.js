...
export const getBundles=(page,items_per_page,country_id,validity_type,company_id,service_category_id,search_tag)=>{
    return async (dispatch)=>{
        ...
        try{
            ...
            const bundle_url=`${base_url}/bundles?page=${page}&per_page=${items_per_page}&country_id=${country_id}&validity_type=${validity_type}&company_id=${company_id}&service_category_id=${service_category_id}&search_tag=${search_tag}`            
            const config = {
                headers: {
